...
#if defined(NDEBUG)
# error "Vaperscoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Vaperscoin Signed Message:\n";
...
//TODO: this isn't identical to vaperscoin reference client.
...
{
    int64_t nSubsidy = 7500 * COIN;
                if(nHeight == 4)
            {
            nSubsidy = 427000000 * COIN;
            }
    ...
    // Subsidy is cut in half every 2 850 000 blocks which will occur approximately every 5 years.
    nSubsidy >>= halvings;
...
static const int64_t nTargetTimespan = 120 ; // Vaperscoin: every 1 minute
static const int64_t nTargetSpacing = 60; // Vaperscoin: 1 minute
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Vaperscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("vaperscoin-scriptch");
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BIP16 was always active in Vaperscoin
...
{
    // Vaperscoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
