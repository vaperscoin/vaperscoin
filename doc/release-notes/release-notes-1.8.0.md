...
Vaperscoin Core 1.8 introduces AuxPoW from block 371,337. AuxPoW is a technology
which enables miners to submit work done while mining other coins, as work
on the Vaperscoin block chain. Vaperscoin Core 1.8 also enables payment protocol
support for Vaperscoin (note that Vaperscoin's implementation of payment protocol is
...
as soon as possible afterwards in order to be able to continue to use Vaperscoin.
Users remaining on the previous client **will not** be able to receive or spend
Vaperscoin (with the exception of with other nodes who have not upgraded).
...
use "vaperscoind" you must run it the first time with the "-reindex" command line
option.
...
AuxPoW benefits Vaperscoin in two ways; firstly, the effective hashrate of the coin is
...
multipools from switching between Vaperscoin and other Scrypt coins.
...
In comparison to the AuxPoW implementation in United Scrypt Coin, Vaperscoin uses a
different coinbase transaction input script format which includes block height in
compliance with BIP0034.

## Payment Protocol

Vaperscoin Core 1.8 adds payment protocol support, which is used to streamline payments
being made to merchants.

This protocol is defined in DIP0070-DIP0072, based on the BIP standards with the same
assigned numbers. DIP standards can be found at https://github.com/vaperscoin/dips/ .
The key differencs between the Bitcoin and Vaperscoin payment protocols are that
the Vaperscoin payment request uses a "genesis" field containing the hash of the network's
...
of Vaperscoin compared to Bitcoin. This should lead to less false positives.

A problem in parsing mangled vaperscoin: URIs under Windows has been resolved, and as
part of this work network detection for vaperscoin: URIs is now more robust.
