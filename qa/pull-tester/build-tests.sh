...
if [ -d "$OUT_DIR" -a -w "$OUT_DIR" ]; then
  set +e
  # Windows:
  cp /Users/pm/github/vapourcoin/win32-build/src/bitcoind.exe $OUT_DIR/bitcoind.exe
  cp /Users/pm/github/vapourcoin/win32-build/src/test/test_bitcoin.exe $OUT_DIR/test_bitcoin.exe
  cp /Users/pm/github/vapourcoin/win32-build/src/qt/bitcoind-qt.exe $OUT_DIR/bitcoin-qt.exe
  # Linux:
  cp /Users/pm/github/vapourcoin/linux-build/src/bitcoind $OUT_DIR/bitcoind
  cp /Users/pm/github/vapourcoin/linux-build/src/test/test_bitcoin $OUT_DIR/test_bitcoin
  cp /Users/pm/github/vapourcoin/linux-build/src/qt/bitcoind-qt $OUT_DIR/bitcoin-qt
  set -e
fi

# Run unit tests and blockchain-tester on Linux:
cd /Users/pm/github/vapourcoin/linux-build
make check

# Run RPC integration test on Linux:
/Users/pm/github/vapourcoin/qa/rpc-tests/wallet.sh /Users/pm/github/vapourcoin/linux-build/src
/Users/pm/github/vapourcoin/qa/rpc-tests/listtransactions.py --srcdir /Users/pm/github/vapourcoin/linux-build/src
# Clean up cache/ directory that the python regression tests create
rm -rf cache

if [ $RUN_EXPENSIVE_TESTS = 1 ]; then
  # Run unit tests and blockchain-tester on Windows:
  cd /Users/pm/github/vapourcoin/win32-build
  make check
fi
